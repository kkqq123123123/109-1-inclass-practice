...
```{r data1}
jsonlite::fromJSON("https://www.dropbox.com/s/t09nrzwx8mw6azn/courses107.json?dl=1") -> courses
ansObj11 <- list()
ansObj12 <- list()
...
map(
  1:8,
  ~{ if( str_detect(source2[[.x]],categories2) == T ) {
   pickBasedOnCategories[[.x]] = TRUE
   } else 
   {
   pickBasedOnCategories[[.x]] = FALSE
   }
 }
)
...
pickExactHistory = {
  str_detect(source3,pattern = "\\b歷史\\b")
   
}

pickHistory = {
  str_detect(source3,pattern = "歷史?")
}
...
{
  part1sub = unlist(str_extract_all(part1,pattern = "[A-Z]{2}[0-9]{1}"))  #取出part1的英文和數字
  patr2sub = unlist(str_extract_all(part2,pattern = "[A-Z]"))  #取出part2的英文
  complete = paste0(part1sub, patr2sub, collapse="、") #用paste0來完成所求
  complete
}
...
#利用str_extract_all來取出每筆課程的應修系級
forMajors = str_extract_all(courses107$應修系級, pattern = "[\u4E00-\u9FFF]{3,20}[0-9][A-Z]?")
